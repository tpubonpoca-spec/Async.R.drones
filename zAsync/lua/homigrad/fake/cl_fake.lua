...
hook.Add("InputMouseApply", "fakeCameraAngles", function(cmd, x, y, angle)
	if IsValid(lply) and lply:InVehicle() then
		local veh = lply:GetVehicle()
		if IsValid(veh) then
			local parent = veh:GetParent()
			local base = IsValid(parent) and parent or veh
			if IsValid(base) and (base.LVS or base.LVSUAV or (base.Base and string.find(tostring(base.Base), "lvs")) or (base:GetClass() and string.find(base:GetClass():lower(), "lvs"))) then
				return
			end
		end
	end
