...
for o in bpy.data.objects:
    if o.type == 'MESH':
        ...
        # Apply decimate modifier to stay under StudioMDL 32768 vertex limit
        if len(o.data.vertices) > 1000 and "8d7a3f28" not in o.name:
...
os.makedirs(os.path.dirname(r"c:\Users\PMC\Desktop\Zdrone-dev\gamepad_src\anims\dabest_decimated.smd"), exist_ok=True)
smd_content = "version 1\n" + "\n".join(nodes_lines) + "\n" + "\n".join(skel_lines) + "\n" + "\n".join(triangles_lines) + "\n"
with open(r"c:\Users\PMC\Desktop\Zdrone-dev\gamepad_src\anims\dabest_decimated.smd", "w", encoding="utf-8") as f:
    f.write(smd_content)

print("DECIMATED_DABEST_SMD_EXPORT_SUCCESS")
