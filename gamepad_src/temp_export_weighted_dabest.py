...
# Assign ValveBiped.Bip01_R_Hand vertex group to all gamepad meshes that have no vertex groups
r_hand_group_name = "ValveBiped.Bip01_R_Hand"

for o in bpy.data.objects:
    if o.type == 'MESH' and o.name != '8d7a3f28ba34eb5':
...
os.makedirs(os.path.dirname(r"c:\Users\PMC\Desktop\Zdrone-dev\gamepad_src\anims\dabest_rigged.smd"), exist_ok=True)
smd_content = "version 1\n" + "\n".join(nodes_lines) + "\n" + "\n".join(skel_lines) + "\n" + "\n".join(triangles_lines) + "\n"
with open(r"c:\Users\PMC\Desktop\Zdrone-dev\gamepad_src\anims\dabest_rigged.smd", "w", encoding="utf-8") as f:
    f.write(smd_content)

print("WEIGHTED_DABEST_SMD_SUCCESS")
