...
# Filter out dummy bone 'c_arms_citizen_mesh_hands' completely
valid_bones = [b for b in arm_obj.data.bones if b.name != 'c_arms_citizen_mesh_hands']
bone_names = [b.name for b in valid_bones]
bone_to_new_id = {name: i for i, name in enumerate(bone_names)}

r_hand_bone_id = bone_to_new_id.get("ValveBiped.Bip01_R_Hand", 6)

nodes_lines = ["nodes"]
for i, b in enumerate(valid_bones):
    parent_id = -1
    if b.parent and b.parent.name in bone_to_new_id:
        parent_id = bone_to_new_id[b.parent.name]
...
target_smd = os.path.join(r"c:\Users\PMC\Desktop\Zdrone-dev\gamepad_src", "anims", "clean_stripped_dabest.smd")
...
for o in mesh_objs:
    ...
    for tri in mesh.loop_triangles:
        triangles_lines.append("material")
        for loop_idx in tri.loops:
            ...
            for g in vert.groups:
                ...
                if vg_name and vg_name in bone_to_new_id:
                    weights.append((bone_to_new_id[vg_name], g.weight))
            if not weights:
...
os.makedirs(os.path.dirname(target_smd), exist_ok=True)
smd_content = "version 1\n" + "\n".join(nodes_lines) + "\n" + "\n".join(skel_lines) + "\n" + "\n".join(triangles_lines) + "\n"
with open(target_smd, "w", encoding="utf-8") as f:
    f.write(smd_content)

print("CLEAN_STRIPPED_DABEST_SMD_EXPORTED")
