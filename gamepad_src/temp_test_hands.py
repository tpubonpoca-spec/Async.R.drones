...
r_hand_bone_id = bone_to_id.get("ValveBiped.Bip01_R_Hand", 7)
spine4_bone_id = bone_to_id.get("ValveBiped.Bip01_Spine4", 1)

nodes_lines = ["nodes"]
for i, b in enumerate(bones):
    if b.name == "c_arms_citizen_mesh_hands":
        parent_id = spine4_bone_id
    elif b.parent:
        parent_id = bone_to_id[b.parent.name]
    else:
        parent_id = -1
...
hands_obj = [o for o in bpy.data.objects if o.name == '8d7a3f28ba34eb5'][0]
target_smd = os.path.join(r"c:\Users\PMC\Desktop\Zdrone-dev\gamepad_src", "anims", "only_hands.smd")

triangles_lines = ["triangles"]
depsgraph = bpy.context.evaluated_depsgraph_get()
eval_o = hands_obj.evaluated_get(depsgraph)
mesh = eval_o.to_mesh()
mesh.calc_loop_triangles()
world_mat = hands_obj.matrix_world
uv_layer = mesh.uv_layers.active.data if mesh.uv_layers.active else None
vg_map = {vg.index: vg.name for vg in hands_obj.vertex_groups}
